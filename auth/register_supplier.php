...
try {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO Supplier (Name, Email, PhoneNo, ContactInfo, Address) 
        VALUES (?, ?, ?, ?, ?)
    ");
    $stmt->execute([$businessName, $email, $phone, $email, $address]);
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (?, ?, 'supplier', ?)
    ");
    $stmt->execute([$email, $hashedPassword, $supplierID]);
